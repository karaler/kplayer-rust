...
impl KPGFactory {
    pub(super) async fn create_playlist(&mut self, cfg: &Root) -> Result<(), KPGError> {
        self.playlist = {
            ...
            for pl in cfg.playlist.iter() {
                let play_model = match pl.play_mode.clone() {
                    ...
                    _ => {
                        return Err(KPGError::new_with_string(
                            ...
                            format!(
                                "invalid playlist play model. name: {}, play_model: {}",
                                pl.name, pl.play_mode
                            ),
                        ));
                    }
                };
                ...
                for res in pl.resource.iter() {
                    match res {
                        ResourceType::ResourceSingle { path } => {
                            let mut media = KPMedia::new(
                                KPMediaResource::SingleSource { path: path.clone() },
                                None,
                                None,
                                None,
                                Some(pl.enable_hardware),
                            );
                        ...
                        ResourceType::ResourceDirectory {
                            ...
                            for file in read_directory_file(directory.clone(), extension.clone())? {
                                let mut media = KPMedia::new(
                                    KPMediaResource::SingleSource { path: file.clone() },
                                    None,
                                    None,
                                    None,
                                    Some(pl.enable_hardware),
                        ...
                        ResourceType::ResourceDetail {
                            ...
                            if let Some(get_stream) = stream {
                                if get_stream.video >= 0 {
                                    expect_streams.insert(
                                        KPAVMediaType::KPAVMEDIA_TYPE_VIDEO,
                                        get_stream.video as u32,
                                    );
                                }
                                if get_stream.audio >= 0 {
                                    expect_streams.insert(
                                        KPAVMediaType::KPAVMEDIA_TYPE_AUDIO,
                                        get_stream.audio as u32,
                                    );
                                }
                                if get_stream.subtitle >= 0 {
                                    expect_streams.insert(
                                        KPAVMediaType::KPAVMEDIA_TYPE_SUBTITLE,
                                        get_stream.subtitle as u32,
                                    );
                                }
                            }
                        ...
                        ResourceType::ResourceGroup {
                            ...
                            let mut media = KPMedia::new(
                                KPMediaResource::MixSource {
                                    video_path: video_path.clone(),
                                    audio_path: audio_path.clone(),
                                    primary_media_type: {
                                        match primary_type {
                                            None => KPAVMediaType::KPAVMEDIA_TYPE_UNKNOWN,
                                            Some(get_type) => match get_type {
                                                ResourceTypeGroupPrimaryType::None => {
                                                    KPAVMediaType::KPAVMEDIA_TYPE_UNKNOWN
                                                }
                                                ResourceTypeGroupPrimaryType::Audio => {
                                                    KPAVMediaType::KPAVMEDIA_TYPE_AUDIO
                                                }
                                                ResourceTypeGroupPrimaryType::Video => {
                                                    KPAVMediaType::KPAVMEDIA_TYPE_VIDEO
                                                }
                                            },
                                        }
                ...
                info!(
                    "create playlist success. playlist: {}, media total: {}",
                    pl.name,
                    playlist.get_media_list().len()
                );
