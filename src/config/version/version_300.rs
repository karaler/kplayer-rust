...
use crate::config::{
    Instance, Output, OutputType, Playlist, ResourceType, Root, Scene, Server, ServerGroup,
    ServerSchema, ServerTokenType, ServerType,
};
...
use serde_json::{from_slice, json, Error, Value};
use std::any::Any;
    ...
    fn parse(&self, data: &Vec<u8>) -> Result<Root, KPGError> {
        let json_value: Value = from_slice(data).map_err(|err| {
            KPGError::new_with_string(
                ...
                format!("parse version 3.0.0 failed. error: {}", err.to_string()),
            )
        })?;

        let mut cfg = Root::default();
        for (key, value) in json_value.as_object().ok_or(KPGError::new_with_str(
            KPGConfigParseFailed,
            "invalid root json format",
        ))? {
            match key.clone() {
                ...
                str if str == String::from("playlist") => {
                    for playlist_item in value.as_array().ok_or(KPGError::new_with_str(
                        KPGConfigParseFailed,
                        "invalid playlist json format",
                    ))? {
                        let mut playlist = Playlist::default();
                        playlist.name = playlist_item
                            .get("name")
                            .ok_or(KPGError::new_with_str(
                                KPGConfigParseFailed,
                                "playlist name can not be empty",
                            ))?
                            .as_str()
                            .ok_or(KPGError::new_with_str(
                                KPGConfigParseFailed,
                                "playlist name invalid json format",
                            ))?
                            .to_string();
                        playlist.enable_hardware = match playlist_item.get("enable_hardware") {
                            None => true,
                            Some(get_enable_hardware) => {
                                get_enable_hardware.as_bool().unwrap_or(true)
                            }
                        };
                        playlist.start_point = match playlist_item.get("start_point") {
                            None => 0,
                            Some(val) => val.as_i64().ok_or(KPGError::new_with_str(
                                KPGConfigParseFailed,
                                "playlist start_point invalid json format",
                            ))? as usize,
                        };
                        playlist.play_mode = match playlist_item.get("play_mode") {
                            ...
                            Some(val) => val
                                ...
                                .ok_or(KPGError::new_with_str(
                                    KPGConfigParseFailed,
                                    "playlist play_mode invalid json format",
                                ))?
                                .to_string(),
                        };
                        playlist.universal_seek = match playlist_item.get("universal_seek") {
                            ...
                            Some(val) => val.as_i64().ok_or(KPGError::new_with_str(
                                KPGConfigParseFailed,
                                "playlist universal_seek invalid json format",
                            ))?,
                        };
                        playlist.universal_end = match playlist_item.get("universal_end") {
                            ...
                            Some(val) => val.as_i64().ok_or(KPGError::new_with_str(
                                KPGConfigParseFailed,
                                "playlist universal_end invalid json format",
                            ))?,
                        };
                        playlist.universal_reverse_seek = match playlist_item.get("universal_seek")
                        {
                            ...
                            Some(val) => val.as_i64().ok_or(KPGError::new_with_str(
                                KPGConfigParseFailed,
                                "playlist universal_reverse_seek invalid json format",
                            ))?,
                        };
                        playlist.universal_reverse_end = match playlist_item.get("universal_end") {
                            ...
                            Some(val) => val.as_i64().ok_or(KPGError::new_with_str(
                                KPGConfigParseFailed,
                                "playlist universal_reverse_end invalid json format",
                            ))?,
                        };

                        let get_resource = playlist_item
                            .get("resource")
                            .ok_or(KPGError::new_with_str(
                                KPGConfigParseFailed,
                                "invalid playlist resource json format",
                            ))?
                            .as_array()
                            .ok_or(KPGError::new_with_str(
                                KPGConfigParseFailed,
                        ...
                        for resource_item in get_resource {
                            let result = match resource_item {
                                Value::String(val) => ResourceSingle {
                                    path: val.to_string(),
                                },
                                Value::Object(val) => {
                                ...
                                _ => {
                                    return Err(KPGError::new_with_string(
                                        KPGConfigParseFailed,
                                        format!("playlist resource invalid json format"),
                                    ));
                                }
                            };
                ...
                str if str == String::from("scene") => {
                    match serde_json::from_value::<Vec<Scene>>(json!(value)) {
                        Ok(get_scenes) => cfg.scene = get_scenes,
                        Err(err) => {
                            return Err(KPGError::new_with_string(
                                KPGConfigParseFailed,
                                format!("scene invalid json format. error: {}", err),
                            ));
                        }
                    }
                }
                str if str == String::from("output") => {
                    for output_item in value.as_array().ok_or(KPGError::new_with_str(
                        KPGConfigParseFailed,
                        "invalid output json format",
                    ))? {
                        let mut output = Output::default();
                        output.name = output_item
                            .get("name")
                            .ok_or(KPGError::new_with_str(
                                KPGConfigParseFailed,
                            ...
                            .ok_or(KPGError::new_with_str(
                                KPGConfigParseFailed,
                                "output name invalid json format",
                            ))?
                            .to_string();
                        output.source = output_item
                            .get("source")
                            .ok_or(KPGError::new_with_str(
                                KPGConfigParseFailed,
                                "output source can not be empty",
                            ))?
                            .as_str()
                            .ok_or(KPGError::new_with_str(
                                KPGConfigParseFailed,
                                "output source invalid json format",
                            ))?
                            .to_string();
                        for group_item in output_item
                            .get("group")
                            .ok_or(KPGError::new_with_str(
                                KPGConfigParseFailed,
                                "output group can not be empty",
                            ))?
                            .as_array()
                            .ok_or(KPGError::new_with_str(
                                KPGConfigParseFailed,
                                "output group invalid json format",
                            ))?
                        {
                            let group = match group_item {
                                Value::String(val) => OutputSingle {
                                    path: val.to_string(),
                                },
                                Value::Object(val) => {
                                    let mut content = HashMap::new();
                                    ...
                                    serde_json::from_value(json!(content)).map_err(|err| {
                                        KPGError::new_with_string(
                                            KPGConfigParseFailed,
                                            format!(
                                                "playlist output invalid json format, error: {}",
                                                err
                                            ),
                                        )
                                    })?
                                ...
                                _ => {
                                    return Err(KPGError::new_with_string(
                                        KPGConfigParseFailed,
                                        format!("playlist output invalid json format"),
                                    ));
                                }
                            };
                ...
                str if str == String::from("server") => {
                    for server_item in value.as_array().ok_or(KPGError::new_with_str(
                        KPGConfigParseFailed,
                        "invalid server json format",
                    ))? {
                        let mut srv = Server::default();
                        srv.name = server_item
                            .get("name")
                            .ok_or(KPGError::new_with_str(
                                KPGConfigParseFailed,
                                "server name can not be empty",
                            ))?
                            .as_str()
                            .ok_or(KPGError::new_with_str(
                                KPGConfigParseFailed,
                                "server name invalid json format",
                            ))?
                            .to_string();
                        srv.target = match server_item
                            .get("target")
                            .ok_or(KPGError::new_with_str(
                                KPGConfigParseFailed,
                                "server target can not be empty",
                            ))?
                            .as_str()
                            .ok_or(KPGError::new_with_str(
                                KPGConfigParseFailed,
                                "server target invalid json format",
                            ))?
                            .to_string()
                        {
                            str if str == "media" => ServerType::Media,
                            str if str == "api" => ServerType::Api,
                            _ => {
                                return Err(KPGError::new_with_string(
                                    KPGConfigParseFailed,
                                    format!("not support server target"),
                                ));
                            }
                        };
                        for group_item in server_item
                            .get("group")
                            .ok_or(KPGError::new_with_str(
                                KPGConfigParseFailed,
                                "server name can not be empty",
                            ...
                            ))?
                        {
                            ...
                            group.name = match group_item.get("name") {
                                ...
                                Some(val) => val
                                    ...
                                    .ok_or(KPGError::new_with_str(
                                        KPGConfigParseFailed,
                                        "server group invalid json format",
                                    ))?
                                    .to_string(),
                            };
                            group.address = match group_item.get("address") {
                                ...
                                Some(val) => val
                                    ...
                                    .ok_or(KPGError::new_with_str(
                                        KPGConfigParseFailed,
                                        "server group address invalid json format",
                                    ))?
                                    .to_string(),
                            };
                            group.schema = match group_item.get("schema") {
                                None => {
                                    return Err(KPGError::new_with_str(
                                        KPGConfigParseFailed,
                                        "server group schema can not be empty",
                                    ));
                                }
                                Some(val) => {
                                    match val.as_str().ok_or(KPGError::new_with_str(
                                        KPGConfigParseFailed,
                                        "server group address invalid json format",
                                    ))? {
                                        str if str == "rtmp" => ServerSchema::Rtmp,
                                        str if str == "http" => ServerSchema::Http,
                                        _ => {
                                            return Err(KPGError::new_with_string(
                                                KPGConfigParseFailed,
                                                format!(
                                                    "server group not support schema. schema: {}",
                                                    val
                                                ),
                                            ));
                                        }
                            ...
                            group.port = match group_item.get("port") {
                                None => {
                                    return Err(KPGError::new_with_str(
                                        KPGConfigParseFailed,
                                        "server group port can not be empty",
                                    ));
                                }
                                Some(val) => val.as_i64().ok_or(KPGError::new_with_str(
                                    KPGConfigParseFailed,
                                    "server group port invalid json format",
                                ))? as u16,
                            };
                ...
                str if str == String::from("instance") => {
                    match serde_json::from_value::<Vec<Instance>>(json!(value)) {
                        Ok(get_instance) => cfg.instance = get_instance,
                        Err(err) => {
                            return Err(KPGError::new_with_string(
                                KPGConfigParseFailed,
                                format!("instance invalid json format. error: {}", err),
                            ));
                        }
                    }
                }
